...
# The runtime has evolved beyond the original broad contextual-investigation
# branch. The current governed form must only dispatch when the Founder intent
# is an execution command. Treat that semantic capability as the migration's
# convergence target so repeated deploys cannot append a second broader branch.
CURRENT_MARKERS = (
    ...
    "v2Intent.intent === V2_FOUNDER_INTENT.EXECUTION_COMMAND && contextualFollowUp.mode === 'CONTEXTUAL_INVESTIGATION'",
    "naturalInvestigationAcknowledgement(contextualFollowUp.target, contextualFollowUp.query || text)",
...
if all(marker in text for marker in CURRENT_MARKERS):
    print('NO_CHANGES_ALREADY_APPLIED:CONTEXTUAL_INVESTIGATION_CONVERGED')
    raise SystemExit(0)
